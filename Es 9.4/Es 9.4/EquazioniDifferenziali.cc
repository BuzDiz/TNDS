...
OscillatoreArmonicoSmorzato::~OscillatoreArmonicoSmorzato() {};

VettoreLineare OscillatoreArmonicoSmorzato::eval(double t, const VettoreLineare& x) const
{
	VettoreLineare sol(2);
		sol.setValue(0, x.getValue(1) );
		sol.setValue(1, sin(m_forzante * t) - x.getValue(1) * m_alpha - x.getValue(0) * pow(m_omega_0, 2) );
 return sol;
}
...
VettoreLineare Pendolo::eval(double t, const VettoreLineare& x) const
{
			...
			sol.setValue(0, x.getValue(1));
      sol.setValue(1, -(g/m_l)*sin(x.getValue(0)));
